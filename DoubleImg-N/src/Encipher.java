...
public class Encipher extends JFrame {
	// 可能太大了一些
	// 128 * 128  可写 1024 个字
	public byte[] temp = new byte[16384];
	public String w;
	public Encipher(){
		...
		char c;
		// 解构
		for(int i = 0; i < t.length && i < 1024; i++){
	...
	public void paint(Graphics g){
		...
		g.drawLine(5, 5, 5 + Cube.length * 128,5);
		g.drawLine(5, 5, 5,5 + Cube.length * 128);
		g.drawLine(5, 5 + Cube.length * 128, 5 + Cube.length * 128,5 + Cube.length * 128);
		g.drawLine(5, 5, 5,5 + Cube.length * 128);
		g.drawLine(5 + Cube.length * 128, 5, 5 + Cube.length * 128, 5 + Cube.length * 128);
