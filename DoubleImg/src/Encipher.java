...
public class Encipher extends JFrame {
	// 可能太大了一些
	// 272 * 267  可写 4539 个字
	public byte[] temp = new byte[72624];
	public String w;
	public Encipher(){
		...
		char c;
		// 解构
		for(int i = 0; i < t.length && i < 4539; i++){
	...
	public void paint(Graphics g){
		...
		g.drawLine(5, 5, 5 + Cube.length * 272,5);
		g.drawLine(5, 5, 5,5 + Cube.length * 267);
		g.drawLine(5, 5 + Cube.length * 267, 5 + Cube.length * 272,5 + Cube.length * 267);
		g.drawLine(5, 5, 5,5 + Cube.length * 267);
		g.drawLine(5 + Cube.length * 272, 5, 5 + Cube.length * 272, 5 + Cube.length * 267);
		g.setColor(Color.BLACK);
		for(int i = 0; i < temp.length; i++){
			if(temp[i] == 1){
				g.setColor(new Color(Math.abs(ra.nextInt() % 200), 255, 255)); // 随机色彩
